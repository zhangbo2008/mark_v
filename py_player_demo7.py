#第七版. 之前跳跃之后点播放会出现视频波动情况. 因为时间轴的颗粒度还不够细. 尝试修改一下看能不能优化.挺奇怪.  # 一种识别高光思路可以用bgm. self.maxi但是改成30000之后结果没变化还是拖拽后会发生视频的加速播放那么半秒左右.这个bug很奇怪. # 改变成修改pos帧数.
...
class myMainWindow(Ui_MainWindow, QMainWindow,QWidget):
    ...
    def __init__(self):
        ...
        self.maxi=3000 #===这个控制整体切分力度. 我感觉把一个视频切1000分足够细化了.
...
#============进度条的控制是我们要做的核心.!!!!!!!!!!!!!!最核心的是下面4个connect的4个函数
        ...
        self.sld_video.ClickedValue3.connect(self.pauseVideo)  # 进度条点击跳转
...
#参考:https://doc.qt.io/qtforpython-5/PySide2/QtWidgets/QAbstractSlider.html#qabstractslider   好资料:https://zhuanlan.zhihu.com/p/75657373
    ...
    def moveSliderbykeyborard(self, position_delta):
        print(123123124435534,position_delta)
        ...
        if 1:  # 开始播放后才允许进行跳转
            ...
            # print(self.player.position(),8888888888888888)
            self.player.setPosition(self.player.position()+int((position_delta / self.maxi) * self.player.duration()))

            self.vidoeLength = self.player.duration() + 0.00000001
            self.lab_video.setText( str(int((self.player.position()) )))
        self.tmpframe = self.player.position()
    ...
    def playVideo(self):
        ...
        self.player.play()
    def pauseVideo(self,a=None): ########=========修改为点暂停按钮就是启动和暂停功能.
        self.sld_video_pressed=False
